...
unit_num = fs*unit_s;  %%计算每次频率计算的点数，默认250*4 = 1000个
...
for subj_i= 1:num_subject  %对每个被试循环
    ...
    for subj_cycle_i =1:cycle_num(subj_i) %对每人每个周期的循环
        ...
        each_mat_mean_psd = zeros(unit_num,subj_cycle_mat_num,19);  %对每个4s求平均 1000*9*19
...
subj_cycle_mat_orgn_data = b(1:21,:); %load的原始数据  19个电极
...
subj_cycle_mat_2 = zeros(19,subj_cycle_mat_length); %subj_cycle_mat_2：subj_cycle_mat_1-参考电压数据 19*457000
subj_cycle_mat_2(1,:) = subj_cycle_mat_1(1,:)-subj_cycle_mat_1(17,:); %FP1相对于A2的信号
subj_cycle_mat_2(2,:) = subj_cycle_mat_1(2,:)-subj_cycle_mat_1(16,:); %FP2相对于A1的信号
subj_cycle_mat_2(3,:) = subj_cycle_mat_1(3,:)-subj_cycle_mat_1(17,:); %F3相对于A2的信号
subj_cycle_mat_2(4,:) = subj_cycle_mat_1(4,:)-subj_cycle_mat_1(16,:); %F4相对于A1的信号
subj_cycle_mat_2(5,:) = subj_cycle_mat_1(5,:)-subj_cycle_mat_1(17,:); %C3相对于A2的信号
subj_cycle_mat_2(6,:) = subj_cycle_mat_1(6,:)-subj_cycle_mat_1(16,:); %C4相对于A1的信号
subj_cycle_mat_2(7,:) = subj_cycle_mat_1(7,:)-subj_cycle_mat_1(17,:); %P3相对于A2的信号
subj_cycle_mat_2(8,:) = subj_cycle_mat_1(8,:)-subj_cycle_mat_1(16,:); %P4相对于A1的信号
subj_cycle_mat_2(9,:) = subj_cycle_mat_1(9,:)-subj_cycle_mat_1(17,:); %O1相对于A2的信号
subj_cycle_mat_2(10,:) = subj_cycle_mat_1(10,:)-subj_cycle_mat_1(16,:); %O2相对于A1的信号
subj_cycle_mat_2(11,:) = subj_cycle_mat_1(11,:)-subj_cycle_mat_1(17,:); %F7相对于A2的信号
subj_cycle_mat_2(12,:) = subj_cycle_mat_1(12,:)-subj_cycle_mat_1(16,:); %F8相对于A1的信号
subj_cycle_mat_2(13,:) = subj_cycle_mat_1(13,:)-mean(subj_cycle_mat_1(16:17,:),1); %Fz相对于A1、A2平均的信号
subj_cycle_mat_2(14,:) = subj_cycle_mat_1(14,:)-mean(subj_cycle_mat_1(16:17,:),1); %Cz相对于A1、A2平均的信号
subj_cycle_mat_2(15,:) = subj_cycle_mat_1(15,:)-mean(subj_cycle_mat_1(16:17,:),1); %Pz相对于A1、A2平均的信号
subj_cycle_mat_2(16,:) = subj_cycle_mat_1(18,:)-subj_cycle_mat_1(17,:); %T3相对于A2的信号
subj_cycle_mat_2(17,:) = subj_cycle_mat_1(19,:)-subj_cycle_mat_1(16,:); %T4相对于A1的信号
subj_cycle_mat_2(18,:) = subj_cycle_mat_1(20,:)-subj_cycle_mat_1(17,:); %T5相对于A2的信号
subj_cycle_mat_2(19,:) = subj_cycle_mat_1(21,:)-subj_cycle_mat_1(16,:); %T6相对于A1的信号
clear subj_cycle_mat_orgn_data;
clear subj_cycle_mat_1;

subj_cycle_mat_3 = reshape(subj_cycle_mat_2',unit_num,[]); %对数据进行维度转换，便于一次性进行功率谱计算  1000*3705=19个(1000*195)
clear subj_cycle_mat_2;

subj_cycle_fft = fft(subj_cycle_mat_3,unit_num);  %求fft  1000*3705=19个(1000*195)
subj_cycle_mat_psd = subj_cycle_fft.*conj(subj_cycle_fft)/(unit_num); %当前mat的功率谱   1000*3705=19个(1000*195)

each_mat_raw_psd = reshape(subj_cycle_mat_psd,size(subj_cycle_mat_psd,1),size(subj_cycle_mat_psd,2)/19,19);  %每个mat最原始的psd
each_cycle_mat_num(i) = size(subj_cycle_mat_3,2)/19; %存储当前周期里mat的unit_num数  1*9 
result_mat_psd = reshape(subj_cycle_mat_psd,size(subj_cycle_mat_psd,1),size(subj_cycle_mat_psd,2)/19,19); %将功率谱转换成三维  1000*unit_num个数*电极
each_mat_mean_psd(:,i,:) = mean(result_mat_psd,2);  %对每个4s求平均    1000*9*19
