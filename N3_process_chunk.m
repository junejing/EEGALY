function N3_process_chunk = N3_process_chunk(Dmeg,Begpts,Endpts, flags)
...
filename='N3_chunk';
mkdir(path(Dmeg),'N3_chunk');
mkdir(path(Dmeg),'N3_chunk_mat');
filepath1=path(Dmeg);
filepath = strcat(filepath1,'\N3_chunk');
filepath2=strcat(filepath1,'\N3_chunk_mat');
numchk = flags.numchunk;
if ~flags.overwr
    while exist(fullfile( ...
                filepath,[flags.prefix,num2str(numchk),'_n3_' fname(Dmeg)]),'file')
        numchk=numchk+1;
    end
end
prefix = [flags.prefix,num2str(numchk),'_n3_'];
N3_process_chunk = fullfile(Dmeg.path,[prefix Dmeg.fname]);
if numchk<10
...
%     end
% end
Dcopy = clone(Dmeg,N3_process_chunk,[Dmeg.nchannels Endpts-Begpts+1 1]);%%%经过这一步产生一个DAT文件
...
filepath=path(Dmeg);
filepath = strcat(filepath,'\N3_chunk');
save(Dcopy) ;     %%%%%%%%%%%%%%%在后面加上分号，就不会显示ans=1；
b=Dcopy(:,:,1);
save(prefix2,'b');
movefile(N3_process_chunk,filepath);
movefile(prefix2,filepath2);
datFile=strcat(N3_process_chunk(1:end-4),'.dat');
