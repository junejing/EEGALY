...
% Last Modified by GUIDE v2.5 25-Jun-2014 09:04:11
...
if isfield(Dmeg,'info')
    if isfield(Dmeg.info,'hour')
         Dmeg.info.hour(3)=ceil(Dmeg.info.hour(3));%秒数向上取整
        Str = [' ' num2str(Dmeg.info.hour(1)) 'h ' num2str(Dmeg.info.hour(2)) 'm ' num2str(Dmeg.info.hour(3)) 's' ];
    end
end
%将Dmeg.info里面的hour数组转换成字符放入str里面，然后在clocktime里面显示
...
if exist('Str','var')
    ...
    handles.Endctime = crc_time_converts(Dmeg.info.hour(1)*60^2 + ...
        Dmeg.info.hour(2)*60 + ...
        Dmeg.info.hour(3) + ...
        nsamples(Dmeg)/fsample(Dmeg)); %%nsamples(Dmeg)是采样点数目15233130，
 %fsample(Dmeg)是采样率500，所以两者相除就是总的秒数，也就是时间，所以这里是计算截止时间
    handles.Endctime(1) = mod(handles.Endctime(1),24);
    handles.Endctime(3) = floor(handles.Endctime(3));
else
    handles.clocktime=0;
end
