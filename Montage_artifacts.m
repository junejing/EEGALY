function varargout = Montage_artifacts(varargin)
% Montage_artifacts MATLAB code for Montage_artifacts.fig
%      Montage_artifacts, by itself, creates a new Montage_artifacts or raises the existing
%      singleton*.
%
%      H = Montage_artifacts returns the handle to a new Montage_artifacts or the handle to
%      the existing singleton*.
%
%      Montage_artifacts('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in Montage_artifacts.M with the given input arguments.
%
%      Montage_artifacts('Property','Value',...) creates a new Montage_artifacts or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Montage_artifacts_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Montage_artifacts_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Montage_artifacts

% Last Modified by GUIDE v2.5 27-Aug-2015 21:26:14
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Montage_artifacts_OpeningFcn, ...
                   'gui_OutputFcn',  @Montage_artifacts_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Montage_artifacts is made visible.
function Montage_artifacts_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Montage_artifacts (see VARARGIN)

% Choose default command line output for Montage_artifacts
...
% UIWAIT makes Montage_artifacts wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Montage_artifacts_OutputFcn(hObject, eventdata, handles) 
...
for ii = 1:num
    if sum==0
       ...
       set(handles.artifacts,'enable','off');
