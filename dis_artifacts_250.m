function varargout = dis_artifacts_250(varargin)
% DIS_ARTIFACTS_250 MATLAB code for dis_artifacts_250.fig
%      DIS_ARTIFACTS_250, by itself, creates a new DIS_ARTIFACTS_250 or raises the existing
%      singleton*.
%
%      H = DIS_ARTIFACTS_250 returns the handle to a new DIS_ARTIFACTS_250 or the handle to
%      the existing singleton*.
%
%      DIS_ARTIFACTS_250('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DIS_ARTIFACTS_250.M with the given input arguments.
%
%      DIS_ARTIFACTS_250('Property','Value',...) creates a new DIS_ARTIFACTS_250 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dis_artifacts_250_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dis_artifacts_250_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dis_artifacts_250

% Last Modified by GUIDE v2.5 06-Sep-2015 10:48:51
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @dis_artifacts_250_OpeningFcn, ...
                   'gui_OutputFcn',  @dis_artifacts_250_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dis_artifacts_250 is made visible.
function dis_artifacts_250_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dis_artifacts_250 (see VARARGIN)
crcdef = crc_get_defaults('one');
set(0,'CurrentFigure',handles.figure1);
warning('off');


% Choose default command line output for dis_artifacts_250
...
if (MarValue==1)
   ...
   set(handles.Artifacts_togglebutton,'Enable','on');
    ...
    artifact_name = [infoldername,'artifact_',dataname];
    artifact_name_exist = exist(artifact_name,'file');
    if (artifact_name_exist == 0)
        data_name_1 = [datapath,'\',dataname(1:end-4)];
        if exist(data_name_1,'file')
            load('Arousal_data.mat');
            data_name = [datapath,'\',dataname(1:end-4),'\',dataname(1:end-4),'_result.mat'];
            load(data_name);
            yushu = mod(length(detection_3),fsample(handles.Dmeg{1})*handles.winsize);
            detection_3(end-yushu+1:end) = [];
            detection = reshape(detection_3,fsample(handles.Dmeg{1})*handles.winsize,[]);
            num_page = size(detection,2);
            Arou.artifact(1:num_page,:) = detection';
            [start_position end_position] = start_end(detection_3);
            for number = 1:length(start_position)
                currentpage_second_start = mod(start_position(number)/fsample(handles.Dmeg{1}),handles.winsize);
                if currentpage_second_start > 0
                    currentpage_start = ceil(start_position(number)/fsample(handles.Dmeg{1})/handles.winsize);
                    Arou.j_artifact(1,currentpage_start) = Arou.j_artifact(1,currentpage_start)+1;
                    down_x = (currentpage_start-1)*handles.winsize + currentpage_second_start;
                end
                currentpage_second_end = mod(end_position(number)/fsample(handles.Dmeg{1}),handles.winsize);
                if currentpage_second_end > 0
                    currentpage_end = ceil(end_position(number)/fsample(handles.Dmeg{1})/handles.winsize);
                    down_y = (currentpage_end-1)*handles.winsize + currentpage_second_end;
                elseif currentpage_second_end == 0
                    currentpage_end = fix(end_position(number)/fsample(handles.Dmeg{1})/handles.winsize);
                    down_y = currentpage_end*handles.winsize + currentpage_second_end;
                end
                up_x = 500;
                up_y = 500;
                Arou.pos_artifact(Arou.j_artifact(1,currentpage_start),:,currentpage_start) = [down_x,up_x,down_y,up_y];
            end
            save('Arousal_data','Arou')
        end
    else
...
% UIWAIT makes dis_artifacts_250 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dis_artifacts_250_OutputFcn(hObject, eventdata, handles) 
...
if (MarValue==1)
    ...
    load('Arousal_data.mat');
    %%%%%画通道F1-O2的标记
    [start_position end_position] = start_end(Arou.artifact(currentpage,:));
    if ~isempty(start_position)
        for number = 1:length(start_position)
            down_x = (start_position(number)/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize);
            down_y = 500;
            up_x = (end_position(number)/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize);
            up_y = 500;
...
%            end
%            [start_position end_position] = start_end(Arou.artifact(currentpage,:));
%            if Arou.artifact(currentpage,1)==1 &&...
%                    length(find(Arou.artifact(currentpage,:))==1) < fsample(handles.Dmeg{1})*handles.winsize && length(start_position) < Arou.j_artifact(1,currentpage);          
%                down_x = (currentpage-1)*handles.winsize+1/fsample(handles.Dmeg{1});
%                down_y = 500;
%                up_x = (index1(1)-1)/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
%                up_y = 500;
%                plot([down_x,up_x],[down_y,up_y],'Color','r','LineWidth',4,'DisplayName','plot_artifact','Tag','1');%连接这两点，作为标记
%                hold on;
%            end
%        elseif ~(any(any(Arou.pos_artifact(:,:,currentpage)))) && ...
%                length(find(Arou.artifact(currentpage,:))==1) == fsample(handles.Dmeg{1})*handles.winsize && Arou.j_artifact(1,currentpage) == 0 && Arou.artifact(currentpage+1,1)==1
%            down_x = (currentpage-1)*handles.winsize+1/fsample(handles.Dmeg{1});
%            down_y = 500;
%            up_x = currentpage*handles.winsize;
%            up_y = 500;
%            plot([down_x,up_x],[down_y,up_y],'Color','r','LineWidth',4,'DisplayName','plot_artifact','Tag','1');%连接这两点，作为标记
%            hold on;
%        elseif ~(any(any(Arou.pos_artifact(:,:,currentpage)))) && ...
...
%            hold on;
%        elseif ~(any(any(Arou.pos_artifact(:,:,currentpage)))) && ...
%                length(find(Arou.artifact(currentpage,:))==1) < fsample(handles.Dmeg{1})*handles.winsize && Arou.j_artifact(1,currentpage) == 0 && Arou.artifact(currentpage,1)==1
%            index1 = (find(Arou.artifact(currentpage-1,:)==0));
%            if isempty(index1)
%                down_x = (currentpage-1)*handles.winsize+1/fsample(handles.Dmeg{1});
%                index = find(Arou.artifact(currentpage,:));
%                down_y = 500;
%                up_x = index(end)/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
%                up_y = 500;
%                plot([down_x,up_x],[down_y,up_y],'Color','r','LineWidth',4,'DisplayName','plot_artifact','Tag','1');%连接这两点，作为标记
%                hold on;
%            end         
...
if ~isempty(Y)
    ...
    elseif (maxpt-minpt)>0.1
        ...
        if down_x == 0
            startpt = fsample(handles.Dmeg{1})*handles.winsize;
        else
        ...
        down_y = 500;
        currentpage_start = (currentpage-1)*handles.winsize+1/fsample(handles.Dmeg{1});
        currentpage_end = currentpage*handles.winsize;
        currentpage_start_fact = (ctshow-handles.winsize/2) + 1/fsample(handles.Dmeg{1});
        currentpage_end_fact = (ctshow+handles.winsize/2); 
        if startpt >= endpt && currentpage_start > currentpage_start_fact
            up_x = endpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage-2)*handles.winsize;
        elseif startpt >= endpt && currentpage_end_fact > currentpage_end
            up_x = endpt/fsample(handles.Dmeg{1})+(currentpage)*handles.winsize;
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
        elseif startpt < endpt && endpt < (currentpage_start_fact-currentpage_start)*fsample(handles.Dmeg{1})
            up_x = endpt/fsample(handles.Dmeg{1})+currentpage*handles.winsize;
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage)*handles.winsize;
        elseif startpt < endpt && endpt < (currentpage_end_fact-currentpage_end)*fsample(handles.Dmeg{1})
            up_x = endpt/fsample(handles.Dmeg{1})+(currentpage-2)*handles.winsize; 
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage-2)*handles.winsize;
        elseif startpt < endpt && currentpage_start == currentpage_start_fact && currentpage_end == currentpage_end_fact
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
             up_x = endpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize; 
        elseif startpt < endpt && endpt < (currentpage_end - currentpage_start_fact)*fsample(handles.Dmeg{1})
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
            up_x = endpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize; 
        elseif startpt < endpt && endpt < (currentpage_end_fact - currentpage_start)*fsample(handles.Dmeg{1})
            down_x = startpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize;
            up_x = endpt/fsample(handles.Dmeg{1})+(currentpage-1)*handles.winsize; 
        end
        up_y = 500;
        uppt=[up_x up_y];
        downpt = [down_x down_y];
...
if (key=='D'||key=='d')
    ...
    for IDex=1:10
        ...
        elseif (max_x1==Arou.pos_artifact(IDex,3,currentpage))&&(y1(1,1)==Arou.pos_artifact(IDex,2,currentpage))%该线段属于当前页
            ...
            elseif (startpt>endpt)&&(min_x1>=(currentpage-1)*handles.winsize)&&(max_x1<=currentpage*handles.winsize)%左利手，当前页内部0-10s之间
        ...
        elseif (min_x1==Arou.pos_artifact(IDex,1,currentpage))
            if abs(Arou.pos_artifact(IDex,1,currentpage) - Arou.pos_artifact(IDex,3,currentpage)) > handles.winsize
                i_artifact = IDex;
                Arou.j_artifact(1,currentpage)=Arou.j_artifact(1,currentpage)-1;
                start_pos = Arou.pos_artifact(IDex,1,currentpage);
                start_1 = (start_pos-(currentpage-1)*handles.winsize)*fsample(handles.Dmeg{1});
                end_pos = Arou.pos_artifact(IDex,3,currentpage);
                end_currentpage = ceil(end_pos/handles.winsize);
                end_1 = (end_pos-(end_currentpage-1)*handles.winsize)*fsample(handles.Dmeg{1});
                for page = currentpage:end_currentpage
                    if page==currentpage
                        Arou.artifact(page,start_1:2501)=0;
                        Arou.pos_artifact(i_artifact:10,:,currentpage)=[Arou.pos_artifact((i_artifact+1):10,:,currentpage);zeros(1,4,1)];
                    elseif page==end_currentpage
                        Arou.artifact(page,1:end_1)=0;
                    else
                        Arou.artifact(page,:)=0;
                    end
                end
            end
        end
...
if (MarValue==1)
    ...
    totalpage = ceil((nsamples(handles.Dmeg{1})/fsample(handles.Dmeg{1}))/handles.winsize);
    %%%%%画通道F1-O2的标记
    detection_all = reshape(Arou.artifact',1,[]);
    start_point = (curtime-handles.winsize/2)*fsample(handles.Dmeg{1})+1;
    end_point = (curtime+handles.winsize/2)*fsample(handles.Dmeg{1});
    [start_position end_position] = start_end(detection_all(start_point:end_point));
    if ~isempty(start_position)
        for number = 1:length(start_position)
            down_x = (curtime-handles.winsize/2) + start_position(number)/fsample(handles.Dmeg{1});
            down_y = 500;
            up_x = (curtime-handles.winsize/2) + end_position(number)/fsample(handles.Dmeg{1});
            up_y = 500;
            plot([down_x,up_x],[down_y,up_y],'Color','r','LineWidth',4,'DisplayName','plot_artifact','Tag','1');%连接这两点，作为标记
        end
    end
